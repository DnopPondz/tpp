    ...
    return (
      <div className="theme-panel">
        <div className="mx-auto max-w-4xl space-y-10 px-6 pb-24 pt-28 text-center lg:px-8">
          <span className="inline-flex w-fit items-center gap-2 rounded-full bg-surface-muted px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-blue-700">
            {t.eyebrow}
          </span>
  ...
  return (
    <div className="theme-surface">
      <div className="mx-auto max-w-6xl space-y-16 px-6 pb-24 pt-28 text-center sm:text-left lg:px-8">
        <header className="theme-panel space-y-6 rounded-[3rem] border border-border-subtle p-10 text-center shadow-2xl transition-colors duration-300 sm:text-left">
          <span className="inline-flex w-fit items-center gap-2 rounded-full bg-surface-muted px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-blue-700">
            {t.eyebrow}
          </span>
        ...
        <section className="grid gap-10 lg:grid-cols-[2fr_1fr]">
          <article className="theme-panel overflow-hidden rounded-[3rem] border border-border-subtle shadow-2xl transition-colors duration-300">
            ...
            <div className="space-y-6 px-8 py-10 text-left">
              ...
              <div className="flex flex-wrap gap-2">
                {featuredPost.tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-surface-muted px-4 py-1 text-xs font-medium text-blue-700"
                  >
                    #{tag}
                  </span>
                ))}
          ...
          <aside className="theme-card flex h-full flex-col gap-6 rounded-[2.5rem] border border-border-subtle p-8 shadow-xl transition-colors duration-300">
          ...
          {paginatedPosts.length > 0 ? (
            <div className="grid gap-8 md:grid-cols-2 xl:grid-cols-3">
              {paginatedPosts.map((post) => (
                <article
                  key={post.slug}
                  className="theme-card group flex h-full flex-col overflow-hidden rounded-[2.25rem] border border-border-subtle shadow-sm transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-xl"
                >
            ...
            <p className="rounded-3xl border border-dashed border-blue-200 bg-surface-muted/60 px-6 py-10 text-center text-base text-blue-700 transition-colors duration-300">
              {t.emptyList}
            </p>
          )}
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <nav aria-label="pagination">
              <ul className="flex items-center gap-2 text-sm font-semibold text-foreground/80">
                <li>
                  {currentPage === 1 ? (
                    <span className="inline-flex items-center rounded-full border border-border-subtle px-4 py-2 text-foreground/60">
                      {t.previous}
                    </span>
                  ) : (
                    <Link
                      href={createPageHref(currentPage - 1)}
                      className="inline-flex items-center rounded-full border border-border-subtle px-4 py-2 transition hover:border-blue-400 hover:bg-surface-muted hover:text-blue-700"
                    >
                      {t.previous}
                    </Link>
                  )}
                      ...
                      <Link
                        ...
                        className={`inline-flex min-w-[2.5rem] items-center justify-center rounded-full border px-3 py-2 transition ${
                          page === currentPage
                            ...
                            : "border-border-subtle hover:border-blue-400 hover:bg-surface-muted hover:text-blue-700"
                        }`}
                      >
                        {page}
                      </Link>
