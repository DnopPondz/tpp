    ...
    <div className="theme-surface">
      <div className="mx-auto max-w-6xl space-y-12 px-6 pb-24 pt-10 lg:px-8">
        <div className="flex items-center gap-3 text-sm text-blue-600">
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-background px-4 py-2 font-medium shadow-sm transition hover:border-blue-400 hover:bg-surface-muted hover:text-blue-700"
          >
        ...
        <div className="grid gap-12 lg:grid-cols-[2fr_1fr]">
          <article className="space-y-10 rounded-[3rem] border border-border-subtle bg-background p-10 shadow-2xl transition-colors duration-300">
            <header className="space-y-6">
              <div className="flex flex-wrap items-center gap-3 text-sm text-foreground/70">
                <span className="rounded-full bg-surface-muted px-3 py-1 font-semibold text-blue-600">
                  {post.category}
                </span>
            ...
            {sectionAnchors.length > 1 && (
              <nav
                ...
                className="rounded-3xl border border-border-subtle bg-surface-muted/80 p-6 transition-colors duration-300"
              >
              ...
              {sectionAnchors.map((section) => (
                <section key={section.id} id={section.id} className="space-y-4">
                  ...
                  {section.bullets && section.bullets.length > 0 && (
                    <ul className="list-disc space-y-2 rounded-2xl bg-surface-muted/80 p-6 text-sm leading-6 text-foreground/80 marker:text-blue-500">
            ...
            <div className="rounded-3xl border border-border-subtle bg-surface-muted/70 p-8 text-sm text-foreground transition-colors duration-300">
              <p className="text-base font-semibold text-blue-700">{t.takeawaysTitle}</p>
              <ul className="mt-4 space-y-3">
                {post.keyTakeaways?.map((item) => (
                  <li key={item} className="flex items-start gap-3">
                    <span className="mt-1 inline-flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-background text-xs font-semibold text-blue-600">
                      ★
                    </span>
                    <span>{item}</span>
                  </li>
            ...
            <div className="flex flex-wrap gap-3 text-sm text-blue-600">
              {post.tags.map((tag) => (
                <span key={tag} className="rounded-full bg-surface-muted px-4 py-1 font-medium">
                  #{tag}
                </span>
              ))}
            </div>
          ...
          <aside className="space-y-8 rounded-[2.5rem] border border-border-subtle bg-background p-8 shadow-xl transition-colors duration-300">
              ...
              <div className="space-y-4">
                {relatedPosts.map((item) => (
                  <Link
                    key={item.slug}
                    href={`/blog/${item.slug}`}
                    className="block rounded-2xl border border-border-subtle bg-surface-muted/70 p-4 transition hover:border-blue-400 hover:text-blue-700"
                  >
            ...
            <div className="space-y-3 rounded-2xl border border-border-subtle bg-surface-muted/70 p-6 text-sm text-foreground transition-colors duration-300">
