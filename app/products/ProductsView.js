  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ...
        className="relative z-10 w-full max-w-3xl overflow-hidden rounded-4xl border border-white/40 bg-background/95 shadow-2xl transition-colors duration-300"
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute right-4 top-4 inline-flex h-9 w-9 items-center justify-center rounded-full border border-border-subtle bg-background text-foreground/70 shadow-sm transition hover:text-foreground/80"
              ...
              <ul className="mt-3 space-y-2 text-sm text-foreground/80">
                {product.details.highlights.map((item) => (
                  <li key={item} className="flex items-start gap-3">
                    <span className="mt-1 inline-flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-surface-muted text-[10px] font-bold text-blue-600">
                      ✓
                    </span>
                    <span>{item}</span>
                  </li>
            ...
            <div>
              ...
              <ul className="mt-3 flex flex-wrap gap-2 text-xs font-semibold text-blue-600">
                {product.details.applications.map((useCase) => (
                  <li
                    key={useCase}
                    className="rounded-full border border-blue-200 bg-surface-muted px-3 py-1"
                  >
                    {useCase}
                  </li>
                ))}
  ...
  return (
    <div className="theme-surface">
      <div className="mx-auto max-w-6xl space-y-20 px-6 pb-24 pt-20 lg:px-8">
        <header className="space-y-5 rounded-[3rem] border border-border-subtle bg-background p-10 text-center shadow-2xl transition-colors duration-300">
        ...
        <section className="grid gap-6 md:grid-cols-2">
          {copy.catalog.map((product) => (
            <article
              key={product.id}
              className="group relative flex h-full flex-col justify-between gap-6 overflow-hidden rounded-4xl border border-border-subtle bg-background/90 p-10 text-foreground/80 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"
              style={{
        ...
        <section className="grid gap-8 rounded-[3rem] border border-border-subtle bg-background p-12 shadow-xl transition-colors duration-300 lg:grid-cols-[1fr_1.2fr]">
          <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {copy.serviceAddOns.map((service) => (
              <div key={service.title} className="rounded-3xl border border-border-subtle bg-surface-muted/80 p-6 text-sm text-foreground/80 transition-colors duration-300">
        ...
        <section className="rounded-[3rem] border border-border-subtle bg-background p-12 shadow-xl transition-colors duration-300">
          <div className="grid gap-6 md:grid-cols-2">
            {sections.products.categories.map((category) => (
              <div key={category.title} className="space-y-3 rounded-3xl border border-border-subtle bg-surface-muted/70 p-6 transition-colors duration-300">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            ...
            <Link
              href="/blog"
              className="inline-flex items-center gap-2 rounded-full border border-blue-300 px-6 py-3 text-sm font-semibold text-blue-600 transition hover:border-blue-400 hover:bg-surface-muted hover:text-blue-700"
            >
              {copy.insightsCta}
              <span aria-hidden>↗</span>
            </Link>
